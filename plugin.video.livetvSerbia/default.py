...
def runiskon(idfile):
  ...
  data = urllib2.urlopen("http://livetvkodiserbia.com/addonxml/iskon_cookie.txt")
  COOKIE = data.read()

  HEADER = "#EXTM3U\n#EXTVLCOPT--http-reconnect=true\n"

  dir_path = os.path.dirname(os.path.realpath(__file__))
  OUTPUT_FILE_1 = os.path.join(functions_dir, "python_IskonTV_by_losmij.m3u")
  #OUTPUT_FILE_2 = "python_IskonTV_by_losmij-rtmp.m3u"
  OUTPUT_FILE_2 = os.path.join(functions_dir, "python_IskonTV_by_losmij-rtmp.m3u")
  INPUT = os.path.join(dir_path, idfile+".txt")
  print dir_path
  # Init
...
#####################################################################
#os.system('tv.bat')
#subprocess.call([dir_path+"\\tv.exe", "-k"])
#subprocess.call([os.path.join(dir_path, "tv.bat")])
#subprocess.call(["C:/kreiraj_listu.exe", "-k"])
#xbmc.executebuiltin('XBMC.RunScript(special:C:\Users\User\AppData\Roaming\Kodi\addons\plugin.video.livetvSerbia\tv.py')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.livetvSerbia-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(livetvSerbia,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(livetvSerbia,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None, ret=False):
  if ret:
   return
  else:
        if url is None:
        ...
        xbmc.executebuiltin("XBMC.Notification(livetvSerbia,New source added.,5000,"+icon+")")
        if not url is None:
...
def parse_m3u_n(data):
    content = data.rstrip()
    match = re.compile(r'#EXTINF:(.+?),(.*?)[\n\r]+([^\r\n]+)').findall(content)
    total = len(match)
    print 'tsdownloader',tsdownloader
#    print 'total m3u links',total
    addon_log('@total m3u links'+str(total))
    ln=''
    for other,channel_name,stream_url in match:
        ...
        ok=addLink(stream_url, channel_name,thumbnail,'','','','','',None,'',total)
        addon_log(str(ok)+'@##@stream_url '+str(stream_url)+"@"+str(channel_name))
        #ln+='\n<item>\n<title>'+channel_name+'</title>\n<link>'+stream_url+'</link>\n</item>'
        #ln+='\n<item>%s\n%s</item>'%(channel_name,stream_url)
    addon_log("@@bla@@@@@")
    addon_log(ln)
    #getData('','',ln)
    #return ln
...
#    print 'doEvalFunction'
    addon.setSetting('save_location', "livetvSerbia")
    lspe=os.path.isfile(functions_dir+'LSProdynamicCode.py')  
    if lspe == False:
      download_file('LSProdynamicCode.py', _Edit.LSProFile, False, False)
    ret_val=''
    if functions_dir not in sys.path:
        sys.path.append(functions_dir)
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(livetvSerbia,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url, ret=True, sl=True):
      if sl:
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('livetvSerbia','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        if ret:
          ret = dialog.yesno('livetvSerbia', 'Do you want to add this file as a source?')
          if ret:
            addSource(os.path.join(addon.getSetting('save_location'), name))
        else:
          addSource(os.path.join(addon.getSetting('save_location'), name), True)
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from livetvSerbia Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to livetvSerbia Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from livetvSerbia Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to livetvSerbia Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(livetvSerbia,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(livetvSerbia,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(livetvSerbia,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(livetvSerbia,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(livetvSerbia,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
#    except: return ret_val
    ...
    tsdownloader=False
    soup = getSoup(os.path.join(functions_dir,url+'.m3u'))
    parse_m3u_n(soup)
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==991:
    addon.setSetting('save_location', "livetvSerbia")
