...
global kontroll
global EPGColour
addon_id = "plugin.video.terminator"
...
supplier = "U2F0dXJudXMgUHJlbWl1bSBUVg=="
ICON = xbmc.translatePath(os.path.join('special://home/addons/' + addon_id, 'iicon.png')) 
...
def peamenyy(params):
    ...
    if channels == 1 and GoDev.mode != 5 and GoDev.mode != 1:
        ...
        plugintools.add_item( action=vod_channels("c2VjdXJpdHlfY2hlY2s="),  title="[COLOR gold][B][I]Terminator TV[/I][/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bGl2ZXR2LnBuZw==")) , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
        plugintools.add_item( action=vod_channels("ZGV0ZWN0X21vZGlmaWNhdGlvbg=="),   title="[COLOR deepskyblue][B]Filmovi[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("dm9kLnBuZw==")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
        ...
        plugintools.add_item( action=vod_channels("bWFpbnRNZW51"),   title="[COLOR white][B]Odrzavanje[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("aWNvbi5wbmc=")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
        plugintools.add_item( action=vod_channels("VG9vbHM="),   title="[COLOR pink][B]Alati i postavke[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("aWNvbi5wbmc=")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
        
    elif channels != 1 and GoDev.mode != 1:
        plugintools.add_item( action=vod_channels("bGljZW5zZV9jaGVjaw=="), title="[COLOR gold][B]1. Step: Enter your username and password[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("aWNvbi5wbmc=")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")), folder=False )	
        plugintools.add_item( action=vod_channels("bGljZW5zZV9jaGVjazI="), title="[COLOR gold][B]2. Step  Login!![/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("aWNvbi5wbmc=")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")), folder=False )	
    ...
    for ShowTitle,start,end,DesC in match:
        ...
        if Realstart > date:
            if Realstart < now:
                ...
                kanalinimi = str('[COLOR red]'+Prefix+'[/COLOR]')+ " - " + '[COLOR gold]'+ShowTitle+'[/COLOR]'
...
def maintMenu(params):
	...
	common.addItem('[COLOR white][B]Automatsko ciscenje[/B][/COLOR]','url',19,ICON,FANART,'')
	common.addItem("[COLOR white][B]Ocisti Cache[/B][/COLOR] - Current Size: " + str(CACHE_SIZE),BASEURL,20,ICON,FANART,'')
	common.addItem("[COLOR white][B]Obrisi Thumbnails [/B][/COLOR] - Current Size: " + str(THUMB_SIZE),BASEURL,22,ICON,FANART,'')
	common.addItem("[COLOR white][B]Obrisi Packages [/B][/COLOR] - Current Size: " + str(PACKAGES_SIZE),BASEURL,23,ICON,FANART,'')
	common.addItem('[COLOR white][B]Azuriranje dodataka[/B][/COLOR]',BASEURL,26,ICON,FANART,'')

def security_check(params):
	plugintools.add_item( action=vod_channels("VFZzZWFyY2g="),   title="[COLOR red][B]Pronadji Kanal[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("U2VhcmNoLWljb24ucG5n")), fanart=os.path.join(LOAD_LIVE,vod_channels("bGl2ZXR2LnBuZw==")) ,  folder=True )
...
def detect_modification(params):
    plugintools.add_item( action=vod_channels("Vk9Ec2VhcmNo"),   title="[COLOR red][B]Pretrazi VOD/TV Serije[/B][/COLOR]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("U2VhcmNoLWljb24ucG5n")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
    plugintools.add_item( action=vod_channels("UmVjZW50bHlBZGRlZA=="),   title="Nedavno dodano" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("dm9kLnBuZw==")), fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
...
def VODsearch(params):
	...
	keyb = xbmc.Keyboard('', '[COLOR white]Pretrazi[/COLOR]')
	keyb.doModal()
	if (keyb.isConfirmed()):
...
def execute_ainfo(params):
	...
	plugintools.add_item( action="",   title="[COLOR white][B]Korisnik: "+Username+"[/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bXlhY2MucG5n")) , fanart=os.path.join(LOAD_LIVE,sync_data("dGhlYXRlci5qcGc=")) , folder=False )
	...
	plugintools.add_item( action="",   title="[COLOR white][B]Vrijedi od: "+Created+"[/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bXlhY2MucG5n")) , fanart=os.path.join(LOAD_LIVE,sync_data("dGhlYXRlci5qcGc=")) , folder=False )
	plugintools.add_item( action="",   title="[COLOR white][B]Max Konekcija: "+Max+"[/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bXlhY2MucG5n")) , fanart=os.path.join(LOAD_LIVE,sync_data("dGhlYXRlci5qcGc=")) , folder=False )
	plugintools.add_item( action="",   title="[COLOR white][B]Aktivnih: "+Current+"[/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bXlhY2MucG5n")) , fanart=os.path.join(LOAD_LIVE,sync_data("dGhlYXRlci5qcGc=")) , folder=False )
